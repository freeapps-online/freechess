...
export function parseVoiceMove(text: string, chess: Chess): string | null {
  ...
  const directMatch = legalMoves.find(m => m.toLowerCase() === input.replace(/\s+/g, ''))
  if (directMatch) return directMatch

  // Handle castling
  if (input.match(/\bcastle\s*(king\s*side|short)\b/) || input.match(/\bcastles?\s*(king|short)\b/) || input === 'castle' || input === 'short castle') {
    if (legalMoves.includes('O-O')) return 'O-O'
  }
  if (input.match(/\bcastle\s*(queen\s*side|long)\b/) || input.match(/\bcastles?\s*(queen|long)\b/) || input === 'long castle') {
    if (legalMoves.includes('O-O-O')) return 'O-O-O'
  }

  // Handle "undo" / "take back"
  if (input.match(/\b(undo|take\s*back|go\s*back)\b/)) return '__undo__'

  // Handle "resign" / "give up"
  if (input.match(/\b(resign|give\s*up|i\s*resign)\b/)) return '__resign__'

  // Handle "new game" / "restart"
  if (input.match(/\b(new\s*game|restart|start\s*over|reset)\b/)) return '__new_game__'
  ...
  const pieceToSquare = input.match(
  ...
  const takesOn = input.match(/\b(?:takes?|captures?)\s*(?:on\s+)?([a-h])\s*([1-8])\b/)
  if (takesOn) {
  ...
  const plainSquare = input.match(/\b([a-h])\s*([1-8])\b/)
  if (plainSquare) {
