...
export default {
  async fetch(req, env) {
    ...
    // POST /api/game/new — create a new game id
    if (url.pathname === '/api/game/new') {
      if (req.method !== 'POST') return new Response('Method not allowed', { status: 405 })
      return Response.json({ id: randomId() })
    }

    // GET /api/game/{id}/ws — upgrade to WebSocket on the DO for this id
    const wsMatch = url.pathname.match(/^\/api\/game\/([a-z0-9-]+)\/ws$/)
    if (wsMatch) {
      const id = wsMatch[1]
      if (!ID_RE.test(id)) return new Response('Invalid game id', { status: 400 })
      const doId = env.GAME.idFromName(id)
      const obj = env.GAME.get(doId)
      return obj.fetch(req)
    }

    // /g/{id} — SPA route. Serve index.html so React-router-style client routing works.
